...
# Phases 1-6: Prerequisites, MI elevation, Auth, SecAdmin, MDC, Sentinel
...
$sanitizedSPNClientId = if ($SERVAPPID -like 'GET-*' -or [string]::IsNullOrWhiteSpace($SERVAPPID)) { "" } else { $SERVAPPID }
$sanitizedSPNSecret = if ($SERVSECRET -like 'GET-*' -or [string]::IsNullOrWhiteSpace($SERVSECRET)) { "" } else { $SERVSECRET }
$sanitizedSPNObjectId = if ($GETSERVICEPRINCIPALOBJECTID -like 'GET-*' -or [string]::IsNullOrWhiteSpace($GETSERVICEPRINCIPALOBJECTID)) { "" } else { $GETSERVICEPRINCIPALOBJECTID }
$sanitizedSPNName = if ($SERVICEPRINCIPALNAME -like 'GET-*' -or [string]::IsNullOrWhiteSpace($SERVICEPRINCIPALNAME)) { "" } else { $SERVICEPRINCIPALNAME }

Write-Host "SPN ClientId provided: $(if ($sanitizedSPNClientId) { 'YES' } else { 'NO' })"
Write-Host "SPN Secret provided: $(if ($sanitizedSPNSecret) { 'YES' } else { 'NO' })"
...
$params = @{
    ...
    odlid = $ODLID
    oduserid = ""
    spnclientid = $sanitizedSPNClientId
    spnsecret = $sanitizedSPNSecret
    spnobjectid = $sanitizedSPNObjectId
    spnname = $sanitizedSPNName
...
# ============================================
# PHASE 1.5: Elevate Managed Identity
# ============================================
Write-Host ""
Write-Host "============================================"
Write-Host "Phase 1.5: Elevate Managed Identity"
Write-Host "============================================"

Update-Status -Phase "01.5-mi" -Status "running"
...
$clientId = $SERVAPPID
$clientSecret = $SERVSECRET

Write-Host "Authenticating to assign GA to MI..."
Write-Host "  SPN ClientId available: $(if ($clientId) { 'YES' } else { 'NO' })"
Write-Host "  TAP available: $(if ($AzureTAP) { 'YES' } else { 'NO' })"

# Auth chain: SPN -> TAP -> Password
if (-not $azLoggedIn -and -not [string]::IsNullOrWhiteSpace($clientId) -and -not [string]::IsNullOrWhiteSpace($clientSecret)) {
    Write-Host "Attempting SPN authentication..."
    az logout 2>$null
    $spnResult = az login --service-principal -u $clientId -p $clientSecret --tenant $AzureTenantID --output none 2>&1
    if ($LASTEXITCODE -eq 0) {
        az account set --subscription $AzureSubscriptionID 2>&1 | Out-Null
        Write-Host "SPN Authentication: SUCCESS"
        $azLoggedIn = $true
        $authMethod = "SPN"
    } else {
        Write-Host "SPN Authentication: FAILED - $spnResult"
    }
}

if (-not $azLoggedIn -and -not [string]::IsNullOrWhiteSpace($AzureTAP)) {
    Write-Host "Attempting TAP authentication..."
    az logout 2>$null
    $tapResult = az login -u $AzureUserName -p $AzureTAP --tenant $AzureTenantID --output none 2>&1
    if ($LASTEXITCODE -eq 0) {
        az account set --subscription $AzureSubscriptionID 2>&1 | Out-Null
        Write-Host "TAP Authentication: SUCCESS"
        $azLoggedIn = $true
        $authMethod = "TAP"
    } else {
        Write-Host "TAP Authentication: FAILED - $tapResult"
    }
}

if (-not $azLoggedIn) {
    Write-Host "Attempting password authentication..."
    az logout 2>$null
    $passResult = az login -u $AzureUserName -p $AzurePassword --tenant $AzureTenantID --output none 2>&1
    if ($LASTEXITCODE -eq 0) {
        az account set --subscription $AzureSubscriptionID 2>&1 | Out-Null
        Write-Host "Password Authentication: SUCCESS"
        $azLoggedIn = $true
        $authMethod = "Password"
    } else {
        Write-Host "Password Authentication: FAILED (likely MFA blocked)"
    }
}

if (-not $azLoggedIn) {
    Write-Host "CRITICAL: All authentication methods failed"
    Update-Status -Phase "01.5-mi" -Status "failed" -Message "All auth methods failed"
    throw "HARD BLOCK: Cannot authenticate. Provide SPN or valid TAP."
}

Write-Host "Auth method used: $authMethod"

# Get MI Principal ID
try {
    $vmName = "LabVM-$DeploymentID"
    Write-Host "Getting Managed Identity for VM: $vmName in RG: $ResourceGroupName"
    ...
    if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($miPrincipalId)) {
        throw "Failed to get MI Principal ID: $miPrincipalId"
    }
    Write-Host "MI Principal ID: $miPrincipalId"
} catch {
    Update-Status -Phase "01.5-mi" -Status "failed" -Message "Cannot get MI Principal ID: $($_.Exception.Message)"
    throw "HARD BLOCK: Cannot get Managed Identity Principal ID"
}

# Assign Owner at subscription scope
Write-Host "Assigning Owner role to MI at subscription scope..."
$ownerRoleId = "8e3af657-a8ff-443c-a75c-2fe8c4bcb635"
$ownerResult = az role assignment create --assignee-object-id $miPrincipalId --assignee-principal-type ServicePrincipal --role $ownerRoleId --scope "/subscriptions/$AzureSubscriptionID" 2>&1
if ($LASTEXITCODE -eq 0) {
    Write-Host "Owner role assigned at subscription scope"
    $ownerAssigned = $true
} elseif ($ownerResult -match "Conflict|already exists") {
    Write-Host "Owner role already assigned"
    $ownerAssigned = $true
} else {
    Write-Host "WARNING: Owner role assignment failed: $ownerResult"
}
...
Write-Host "Assigning Global Administrator role to MI..."
$gaRoleTemplateId = "62e90394-69f5-4237-9190-012177145e10"
$gaBodyFile = "$env:TEMP\ga-body.json"
@{
    roleDefinitionId = $gaRoleTemplateId
    principalId = $miPrincipalId
    directoryScopeId = "/"
} | ConvertTo-Json | Set-Content $gaBodyFile -Encoding UTF8 -Force

$gaResult = az rest --method POST --url "https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignments" --body "@$gaBodyFile" 2>&1
Remove-Item $gaBodyFile -Force -ErrorAction SilentlyContinue

if ($LASTEXITCODE -eq 0) {
    Write-Host "Global Administrator assigned to MI"
    $gaAssigned = $true
} elseif ($gaResult -match "conflicting object|already exists") {
    Write-Host "Global Administrator already assigned"
    $gaAssigned = $true
} else {
    Write-Host "WARNING: GA assignment failed: $gaResult"
    $gaVerify = az rest --method GET --url "https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignments" --query "value[?principalId=='$miPrincipalId' && roleDefinitionId=='$gaRoleTemplateId'].id" -o tsv 2>&1
    if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($gaVerify)) {
        Write-Host "GA was already assigned (verified via GET)"
        $gaAssigned = $true
    } else {
        Write-Host "CRITICAL: GA assignment truly failed"
    }
}

# Switch to MI auth
az logout 2>$null
Write-Host "Waiting 15s for role propagation..."
Start-Sleep -Seconds 15

az login --identity --output none 2>&1
az account set --subscription $AzureSubscriptionID --output none 2>&1

# Validate MI can access Entra ID (proves GA is active)
$testUser = az ad user list --query "[0].displayName" -o tsv 2>&1
if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($testUser)) {
    Write-Host "MI can access Entra ID: verified (found user: $testUser)"
    $miElevated = $true
    $gaAssigned = $true
} else {
    Write-Host "MI cannot access Entra ID yet, retrying after 15s..."
    Start-Sleep -Seconds 15
    az login --identity --output none 2>&1
    $retryUser = az ad user list --query "[0].displayName" -o tsv 2>&1
    if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($retryUser)) {
        Write-Host "MI Entra ID access confirmed on retry"
        $miElevated = $true
        $gaAssigned = $true
    } else {
        Write-Host "Extended retry: waiting 30s more..."
        Start-Sleep -Seconds 30
        az login --identity --output none 2>&1
        $finalUser = az ad user list --query "[0].displayName" -o tsv 2>&1
        if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($finalUser)) {
            Write-Host "MI Entra ID access confirmed on extended retry"
            $miElevated = $true
            $gaAssigned = $true
        } else {
            Write-Host "WARNING: GA still not active. Owner works for Azure RBAC but Entra ID ops will fail."
            $miElevated = $true
        }
    }
}
...
$phaseMsg = "Owner:$(if ($ownerAssigned){'YES'}else{'NO'}) GA:$(if ($gaAssigned){'YES'}else{'NO'}) Auth:$authMethod"
...
if (-not $authMethod) {
    $currentParams = Get-Content $ParametersFile | ConvertFrom-Json
    if ($currentParams.spnclientid -and $currentParams.spnsecret) {
        Write-Host "Attempting SPN authentication..."
        az login --service-principal -u $currentParams.spnclientid -p $currentParams.spnsecret --tenant $AzureTenantID 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
            az account set --subscription $AzureSubscriptionID 2>&1 | Out-Null
            Write-Host "SPN Authentication: SUCCESS"
...
$currentParams = Get-Content $ParametersFile | ConvertFrom-Json
$authMethod = $currentParams.authmethod

if ($authMethod -eq "ManagedIdentity") {
    az login --identity 2>&1 | Out-Null
    az account set --subscription $AzureSubscriptionID 2>&1 | Out-Null
} elseif ($currentParams.spnclientid -and $currentParams.spnsecret) {
    az login --service-principal -u $currentParams.spnclientid -p $currentParams.spnsecret --tenant $currentParams.tenantid --output none 2>$null
    az account set --subscription $currentParams.subid --output none
}
...
# PHASE 13: Execute Attack Simulations
# ============================================
Invoke-Phase -PhaseName "13-attacks" -ScriptPath "$ScriptsPath\08attacks\launch-attacks.ps1" -Parameters @{
    ParametersFile = $ParametersFile
} -MaxRetries 2
...
Write-Host "  GA Assigned: $($finalParams.migaassigned)"
